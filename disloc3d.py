...
def dc3d_wrapper(m,slip,obs_pts):
    ...
    elif c >= W * np.sin(dip) and np.array_equal(z, -np.abs(z)):
        pass
    else:
...
def demo():
    ...
    # Reshape the displacement for plotting (e.g., x-component of displacement)
    Z = dmag.reshape(num, num)  # Adjust disp[:, 0] to the displacement component you need
